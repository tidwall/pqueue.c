    ...
    int (*compare)(const void *a, const void *b, void *udata), void *udata)
{
    ...
    queue->elsize = elsize;
    queue->malloc = malloc;
    queue->realloc = realloc;
    queue->free = free;
    queue->compare = compare;
    queue->udata = udata;
    queue->cap = 8;
    queue->len = 0;
    queue->items = malloc(elsize*(queue->cap+1));
    if (!queue->items) {
        free(queue);
        return NULL;
    }
...
struct pqueue *pqueue_new(size_t elsize, 
    int (*compare)(const void *a, const void *b, void *udata), void *udata)
{
    return pqueue_new_with_allocator(elsize, malloc, realloc, free, compare, 
        udata);
}

PQUEUE_EXTERN
void pqueue_free(struct pqueue *queue) {
    if (queue->items) queue->free(queue->items);
    queue->free(queue);
}
...
static bool push(struct pqueue *queue, const void *item) {
    if (queue->len == queue->cap) {
        ...
        char *items = queue->realloc(queue->items, queue->elsize*(cap+1));
...
static int compare(struct pqueue *queue, size_t i, size_t j) {
    return queue->compare(get(queue, i), get(queue, j), 
        queue->udata);
}
